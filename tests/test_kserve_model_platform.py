...
from kserve_model_platform.demo_cockpit import build_judge_demo_cockpit
...
from kserve_model_platform.hpa_scale_to_zero import build_hpa_scale_to_zero_plan
...
from kserve_model_platform.inplace_resize import build_inplace_resize_plan
...
from kserve_model_platform.multi_team_readiness import build_multi_team_readiness_plan
...
from kserve_model_platform.suspended_job_resources import build_suspended_job_resource_plan
...
class KServeModelServingPlatformTest(unittest.TestCase):
    ...
    def test_dra_resource_health_status_plan_and_kubernetes_assets_exist(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(report["passed"])
            self.assertEqual(report["recommended_action"], "enable_kserve_dra_resource_health_runbook")
            ...
            self.assertTrue(any(event["workload"] == "rollback-smoke-probe" for event in report["device_health_events"]))
        ...
        for expected in ["DeviceTaintRule", "allocatedResourcesStatus", "ResourceHealthStatus", "resourceclaims", "kube_resourceclaim_status_devices"]:
            self.assertIn(expected, manifest)
        for expected in ["KServe", "ResourceClaim", "PodResourcesLister", "rollback smoke"]:
            self.assertIn(expected, docs)
    ...
    def test_advanced_device_sharing_plan_and_kubernetes_assets_exist(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(report["passed"])
            self.assertEqual(report["recommended_action"], "enable_kserve_dra_advanced_device_sharing_policy")
        ...
        for expected in ["prioritizedList", "partitionable", "capacity:", "bindingConditions", "KServeDRADeviceBindingWaitHigh"]:
            self.assertIn(expected, manifest)
        for expected in ["KServe", "prioritized", "Partitionable", "binding conditions"]:
            self.assertIn(expected, docs)
    ...
    def test_admin_access_diagnostics_plan_and_kubernetes_assets_exist(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(report["passed"])
            self.assertEqual(report["recommended_action"], "enable_kserve_dra_admin_access_diagnostics")
            self.assertEqual(report["feature"]["feature_gate"], "DRAAdminAccess")
            self.assertTrue(any("LatestRolledoutRevision" in item["owner_action"] for item in report["diagnostics"]))
            self.assertTrue((root / "reports" / "admin_access_diagnostics_plan.json").exists())
        for expected in ["resource.kubernetes.io/admin-access", "adminAccess: true", "mlops-serving-dra-admin", "KServeDRAAdminAccessClaimRunningTooLong"]:
            self.assertIn(expected, manifest)
        for expected in ["KServe DRA AdminAccess Diagnostics", "AdminAccess", "Least-privilege RBAC", "model-cache state"]:
            self.assertIn(expected, docs)
    ...
    def test_inplace_resize_plan_and_kubernetes_assets_exist(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(report["passed"])
            self.assertEqual(report["recommended_action"], "enable_kserve_inplace_resize_controls")
        ...
        for expected in ["--subresource resize", "resizePolicy", "InPlaceOrRecreate", "PodResizePending", "KServeInPlaceResizeInProgressTooLong"]:
            self.assertIn(expected, manifest)
        for expected in ["KServe In-Place Pod Resize Controls", "pods/resize", "PodResizePending", "InPlaceOrRecreate"]:
            self.assertIn(expected, docs)
    ...
    def test_asset_partitioning_plan_and_airflow_assets_exist(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(report["passed"])
            self.assertEqual(report["recommended_action"], "enable_airflow_asset_partitioning_for_serving_rollouts")
            self.assertIn("PartitionedAssetTimetable", report["features"]["timetables"])
            self.assertTrue(any(flow["partition_key"] == "model_version:route_generation:window" for flow in report["flows"]))
    ...
    def test_multi_team_readiness_plan_and_airflow_config_exist(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(report["passed"])
            self.assertEqual(report["recommended_action"], "prepare_airflow_multi_team_serving_isolation")
            self.assertEqual(report["team"]["team_name"], "ml-serving")
        ...
        for expected in [
            ...
            "AIRFLOW_VAR__ML_SERVING",
        ]:
            self.assertIn(expected, docs)
        for expected in ["team_name", "ml-serving", "multi_team = True", "LocalExecutor;ml-serving=KubernetesExecutor"]:
            self.assertIn(expected, config)
    ...
    def test_pending_workload_visibility_plan_and_kubernetes_assets_exist(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(report["passed"])
            self.assertEqual(report["recommended_action"], "enable_serving_kueue_pending_workload_visibility")
            self.assertEqual(report["feature"]["api_group"], "visibility.kueue.x-k8s.io/v1beta2")
            self.assertTrue(any(item["local_queue"] == "online-route-smoke" for item in report["pending_workloads"]))
        ...
        for expected in ["visibility.kueue.x-k8s.io", "clusterqueues/pendingworkloads", "localqueues/pendingworkloads", "kueue_admission_wait_time_seconds", "kueue_cluster_queue_resource_pending"]:
            self.assertIn(expected, manifest)
        for expected in ["Kueue Pending Workload Visibility", "VisibilityOnDemand", "route smoke", "pending workload"]:
            self.assertIn(expected, docs)
    ...
    def test_workload_aware_scheduling_plan_and_kubernetes_assets_exist(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(report["passed"])
            self.assertEqual(report["recommended_action"], "prepare_workload_aware_scheduling_for_serving_analysis")
            ...
            self.assertTrue(any(item["pod_group"] == "shadow-replay-pg" for item in report["workloads"]))
            self.assertTrue((root / "reports" / "workload_aware_scheduling_plan.json").exists())
        for expected in ["Kubernetes Workload-Aware Scheduling", "PodGroup", "WorkloadWithJob", "ResourceClaim", "InferenceService predictors"]:
    ...
    def test_control_plane_diagnostics_plan_and_kubernetes_assets_exist(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(any(controller["name"] == "kserve-route-controller" for controller in report["controllers"]))
        ...
        for expected in ["Control Plane Diagnostics", "/statusz", "/flagz", "PSI", "native histogram"]:
            self.assertIn(expected, docs + manifest)
        for expected in ["ServiceMonitor", "PrometheusRule", "KServeRouteControllerCacheStale", "KubeletPSIMemoryStallHigh", "NativeHistogramStorageBudgetHigh"]:
            self.assertIn(expected, manifest)
    ...
    def test_memory_qos_plan_and_kubernetes_assets_exist(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(any(workload["name"] == "champion-predictor" and workload["qos_class"] == "Guaranteed" for workload in report["workloads"]))
        ...
        for expected in ["Memory QoS", "TieredReservation", "memory.min", "memory.low", "memory.high", "cgroup v2"]:
            self.assertIn(expected, docs + manifest)
        for expected in ["KubeletConfiguration", "MemoryQoS", "PrometheusRule", "KServeMemoryQoSThrottlingHigh", "KServeMemoryQoSPSIPressureHigh"]:
            self.assertIn(expected, manifest)
    ...
    def test_hpa_scale_to_zero_plan_and_kubernetes_assets_exist(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(report["passed"])
            self.assertEqual(report["recommended_action"], "enable_hpa_scale_to_zero_for_async_serving_workers")
            ...
            self.assertFalse({"credit-risk-router", "champion-predictor"} & {workload["name"] for workload in report["scale_to_zero_workloads"]})
        ...
        for expected in ["HPA Scale To Zero", "HPAScaleToZero", "minReplicas: 0", "External", "cold-start"]:
            self.assertIn(expected, docs + manifest)
        for expected in ["HorizontalPodAutoscaler", "autoscaling/v2", "type: Object", "KServeScaleToZeroWakeupFailed", "KServeScaleToZeroColdStartBudgetExceeded"]:
            self.assertIn(expected, manifest)
    ...
    def test_suspended_job_resource_plan_and_kubernetes_assets_exist(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(report["passed"])
            self.assertEqual(report["recommended_action"], "enable_suspended_job_resource_mutation_for_queued_serving_jobs")
        ...
        for expected in ["Suspended Job Resource Mutation", "MutablePodResourcesForSuspendedJobs", "spec.suspend", "CPU", "GPU", "HTTPRoute"]:
            self.assertIn(expected, docs + manifest)
