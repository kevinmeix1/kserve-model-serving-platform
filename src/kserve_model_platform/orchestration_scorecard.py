...
def build_orchestration_scorecard(
    ...
    *,
    ...
    checks = [
        ...
        ("kserve_llm_inference_readiness", _present(content, "llm_inference_readiness_plan.json", "LLMInferenceService", "vLLM") and _present(content, "time_to_first_token_ms", "time_per_output_token_ms", "ModelCar"), "KServe LLMInferenceService readiness covers vLLM, OCI ModelCar artifacts, endpoint-picker routing, TTFT/TPOT budgets, and LoRA capacity guardrails"),
        ("kserve_transformer_explainer_topology", _present(content, "transformer_explainer_readiness_plan.json", "transformer-container", "--enable_predictor_health_check") and _present(content, "ServingRuntime", "async-risk-explainer", "KServeTransformerPredictorHealthCheckFailing"), "KServe Predictor, Transformer, and async Explainer readiness covers independent scaling, predictor health gates, bounded sync latency, and fallback routes"),
        ...
        ("airflow_stateful_orchestration", all(token in content for token in ["airflow_stateful_orchestration_plan.json", "task_state_store", "asset_state_store", "RollupMapper", "FanOutMapper", "PartitionedAtRuntime", "apache-airflow==3.3.0", "airflow-sdk-contract"]), "Airflow 3.3 serving DAGs preserve rollout identity with scoped state stores, bounded partition mapping, and a real SDK parse gate"),
    ...
    report = {
        ...
        "research_basis": [
            ...
            "KServe LLMInferenceService, llm-d/vLLM serving patterns, OCI ModelCar storage, TTFT/TPOT budgets, and LoRA adapter capacity guardrails for generative inference",
            "KServe Predictor, Transformer, and Explainer data-plane roles with separate-by-default Transformer/ Predictor deployments, optional collocation, ServingRuntime contracts, and async explainer scaling",
            ...
            "Kubernetes v1.36 ConstrainedImpersonation with separate identity and impersonate-on action authorization for least-privilege serving debugging",
